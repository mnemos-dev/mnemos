...
- `skills/mnemos-refine-transcripts/SKILL.md` — mirror template change (junctioned, same repo)
...
- Modify: `skills/mnemos-refine-transcripts/SKILL.md` (mirror, junctioned — manually check whether it propagates automatically)
...
tags: [session-log, <2-6 relevant technical tags, lowercase>]
...
date: YYYY-MM-DD
project: <Wing — per the mapping below>
cwd: <absolute Windows path, working directory the transcript originated from>
tags: [session-log, <2-6 relevant technical tags, lowercase>]
duration: <~Xs / ~Xm / ~Xh — rough estimate from transcript length>
---
```
...
In the JSONL lines, each message has a `"cwd":"C:\\..."` field
(Claude Code records it on every turn). Extract the value from the
first non-metadata line and write it as a Windows absolute path:
...
If the JSONL has no `"cwd"` field (older Claude Code version, rare), skip
the field entirely — the briefing hook silently filters out sessions without cwd.
```
...
- Read its "Özet" (Summary), "Alınan Kararlar" (Decisions Made), "Sorunlar" (Issues), "Sonraki Adımlar" (Next Steps)
  sections verbatim.
...
- Keep all sessions' Özet (Summary) + Alınan Kararlar (Decisions) sections.
- Drop "Sonraki Adımlar" (Next Steps) for older sessions (keep for last 3 only).
...
```markdown
**Aktif durum:** <1-2 sentences — what this cwd is focused on as of today, who/what
is waiting>

**Geçerli kararlar:**
- <date> — <short decision> [[drawer-wikilink]]
- <date> — <short decision>
- (still in effect, ordered oldest to newest)

**Revize/iptal edilen kararlar:**
- <old-date> "<old decision>" → <new-date> "<new decision>". Rationale: <short>
- (skip this section entirely if there are no revisions)

**Açık meseleler:**
- <unresolved problem or TODO> [[drawer-wikilink]]
- (skip this section if no pending work)

**Sırada:**
- <likely work for the next session, prediction>
```

### Rules

- **Wikilinks:** Each drawer reference in `[[drawer-slug]]` format (slug =
  drawer filename, without `.md`). Clickable in Obsidian.
- **Contradiction detection:** If an old decision was later revised, show it
  explicitly in the "Revize/iptal" section. As the LLM, derive this from the
  chronological flow of decisions — catch transitions like "we will do X" →
  "we dropped X, went back to Y".
- **Language:** If the user's sessions are in Turkish, the briefing is in Turkish.
  If sessions are in English, English. If mixed, pick the dominant language.
- **Tone:** Technical, objective. No "I hope..." / "I think...". Be honest about
  gaps: if info is missing, write "unknown", don't fabricate.
- **Empty cwd:** If there are no matching sessions:
...
Markdown body only to stdout. Don't write frontmatter (the wrapper adds it).
Don't write unnecessary metadata, preamble, or "here is your briefing:"-style
explanations. Start directly with `**Aktif durum:**`.
```
...
```markdown
- [~] **4.3 Skill-recall** *(~5h, split to two ships)* — *(started 2026-04-23)*
...
- **4.3.1 Explicit `/mnemos-recall <query>` skill** *(~2h, second ship)* —
  cross-context edge case (a GYP procurement reminder while in farcry cwd).
  Own brainstorm + spec + plan, separate ship.
