...
## Pre-flight state (verified 2026-04-29 mid-day, refreshed after SessionEnd-persistence fix)

- `origin/main` == HEAD == `2cf43f9` (12 commits ahead of dawn-of-day on 2026-04-28; the last two are the stale-OK skip supersede fix `7959ada` and the SessionEnd persistence + X-close coverage fix `2cf43f9`, both rolling into v1.2.1)
- Test suite: **556 pass**, 2 skipped, 3 deselected (+11 vs initial 545: 6 new in `test_session_end_supersede.py`, 4 new for SessionEnd persistence/X-close coverage, 1 prior test renamed-and-retargeted; +3 net vs 553)
...
- Stray `Gemini_Generated_Image_hcsek3hcsek3hcse.png` untracked in repo root (never staged; ignore for publish)
...
- v1.2.1 stale-OK skip supersede (4th class of refine data loss, 2026-04-28)
- v1.2.1 SessionEnd persistence + X-close coverage (5th class — brief_regen + identity_refresh DEVNULL, sync fallback gate widened, LLM preamble strip, 2026-04-29)
...
# Expect: 556 passed, 2 skipped, 3 deselected
```
...
git tag v1.2.1 -a -m "v1.2.1 — Refine race + identity isolation + stale-OK supersede + SessionEnd persistence + X-close coverage" 2cf43f9
...
gh release create v1.2.1 \
  --title "v1.2.1 — Refine race + identity isolation + SessionEnd persistence hot-fix" \
...
2. Verify pre-flight state still matches:
   ...
   - `python -m pytest tests/ -q` still 556 pass
