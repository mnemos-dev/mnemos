...
- [ ] **4.1.P9 Phase A md ↔ xlsx jsonl-row sync** *(~30 min)* —
  2026-04-22 smoke'unda tespit: bir Session md Phase A'da mine edildiğinde,
  o md'nin arkasındaki JSONL'in xlsx satırı (backfill'den gelmiş) mine
  bilgisiyle güncellenmiyor → xlsx'te aynı iş iki row'da görünüyor (jsonl
  `mined_outcome=PENDING`, md `mined_outcome=OK`). Fix: Phase A loop'unda
  session_md için refine ledger'da ters arama yap (hangi JSONL bu session_md'yi
  üretti?), bulunursa o jsonl row'una da aynı mine bilgilerini yaz.
  Alternatif: xlsx'e yeni md row eklemek yerine, refine ledger match eden
  JSONL row'unu güncelle (single-source-of-truth per logical work unit).
- [ ] **4.1.P8 Wrapper → bg subprocess spawn reliability** *(~1h)* —
  2026-04-22 real-vault test'te kullanıcı yeni Claude Code session
  açtığında wrapper `.mnemos-active-sessions/<uuid>.json` marker'ını
  yazdı ama spawn ettiği `auto_refine_background` bg subprocess
  **sessizce öldü** — hook.log ve hook-status.json hiç güncellenmedi.
  Aynı bg, manuel `python -m mnemos.auto_refine_background ...`
  çağrısıyla sorunsuz çalıştı (Phase A mine'ı başlattı). Windows
  `DETACHED_PROCESS + CREATE_NEW_PROCESS_GROUP + stdout=DEVNULL`
  kombinasyonunda Python child process handle inheritance bazen
  crash oluyor — stdout DEVNULL olduğu için import/startup error
  tamamen görünmez kalıyor. Fix options: (a) bg stdout'u `<vault>/.mnemos-bg-last.log`'a yönlendir (rotating, append), (b) wrapper
  spawn sonrası 100ms bekle + bg process handle'ını stat'leyip
  hâlâ canlı mı kontrol et, değilse wrapper hata log'una yaz,
  (c) bg subprocess'in ilk satırı log'a "booted <ts>" yaz →
  wrapper 500ms sonra log'u doğrula, yoksa retry.
