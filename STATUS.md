...
**Last updated:** 2026-05-01 — **v1.2.1 published to PyPI** (https://pypi.org/project/mnemos-dev/1.2.1/, GitHub release https://github.com/mnemos-dev/mnemos/releases/tag/v1.2.1, annotated tags `v1.2.0` + `v1.2.1`). Earlier same day: v1.2.0 (locale-aware output) + v1.2.1 (refine-pipeline race + identity isolation hot-fix + stale-OK skip supersede + **SessionEnd persistence + X-close coverage**) all shipped to origin/main. v1.2.1 took on extra scope after empirical bootstrap pilot surfaced three more bugs (ANTHROPIC_API_KEY env strip missing, parent-cwd + SessionStart hook contamination of `claude --print` subprocess, OUTPUT-section prompt too lax), and a fourth class of refine-pipeline data loss landed late in the day: **stale-OK skip on SessionEnd**. When a JSONL had been refined while still active (sibling SessionStart's `auto_refine` picked it during an idle window), any later `/exit` for that session silently skipped re-refining via `claim_jsonl_for_refine`'s ledger=OK gate — losing every byte typed after the premature refine. Reproduced on the day's planning session (~6h of post-refine work survived only in the raw JSONL). Fix: `supersede_stale_refine_if_needed` in `mnemos/session_end_hook.py` runs before the claim; when `JSONL.mtime > Session/.md.mtime + 60s` it renames the stale Session to `.bak-superseded-<utc>` and drops the OK row so claim grants. Manual recovery refreshed the lost Session as `2026-04-28-mnemos-v1-2-1-shipped-identity-bootstrap-pypi-deferred.md`. Identity bootstrap empirically validated end-to-end: pilot on 10 most-recent Sessions then full run on 90 Sessions, both produced clean seven-section locale-aware profiles (TR headers + TR body since kasamd is Turkish-dominant). Total commits ahead of origin/main: 0 (`feat/identity-bootstrap-limit` branch == origin/main; all 6 commits pushed via `HEAD:main`, latest is `2cf43f9`). Test suite **556 pass** (+11 vs 545: 6 new in `test_session_end_supersede.py`, 4 new for SessionEnd persistence/X-close coverage, 1 prior test renamed-and-retargeted, plus 2 fixed test mocks for the new `vault` kwarg in `_run_refine` and `_run_brief_regen`). v1.1.0 GitHub release still live on 2026-04-27 ([tag](https://github.com/mnemos-dev/mnemos/releases/tag/v1.1.0)); v1.1.0 + v1.2.0 are git-tag-only milestones (PyPI v1.x line starts at 1.2.1 per the "Why skip" rationale in the publish plan). End-to-end smoke confirmed pre-publish: briefing cache regen ran today at 10:56 against this cwd, body starts with `**Current State:**`, contains `2cf43f9`. Bumps shipped in `074a066` (chore(release): bump version to 1.2.1).
...
**Released:** `v1.1.0` — SessionEnd-driven memory architecture, GitHub release live, **git-tag only (skipped on PyPI per v1.2.1 publish plan)**
**Released:** `v1.2.0` — Locale-aware output, **git-tag only (skipped on PyPI per v1.2.1 publish plan)**
**Released:** `v1.2.1` — Refine-pipeline race fix (per-JSONL filelock + ledger normalize CLI) + identity isolation fix (env-strip + neutral cwd + hook re-entry guard) + `bootstrap --limit N` pilot mode + stale-OK skip supersede (SessionEnd defense-in-depth) + SessionEnd persistence + X-close coverage hardening. **PyPI: https://pypi.org/project/mnemos-dev/1.2.1/** · **GitHub: https://github.com/mnemos-dev/mnemos/releases/tag/v1.2.1**
...
|---|---|---|---|
| F1 Constants module | 0 | skipped | 2-consumer scope small enough to inline |
| F2 Refined Session schema | 2 | done | refine-transcripts.md locale-aware language rule + EN/TR translation table |
| F3 Identity Layer schema | 4 | done | bootstrap.md + refresh.md + skill prompt locale-aware; `_REFRESH_PROMPT_TEMPLATE` rules translated to English; refresh preserves existing profile's header language |
...
- **v1.2.1 SessionEnd persistence + X-close coverage hardening (2026-04-29)**: user reported briefing inject always lagged one /exit cycle. Root cause: `_run_brief_regen` and `_run_identity_refresh_if_due` in the SessionEnd worker piped skill subprocess stdout to `subprocess.DEVNULL` — the briefing body and refreshed identity body were both **discarded**. Cwd cache and `_identity/L0-identity.md` only ever refreshed via the next SessionStart's bg catchup (by-design async), so SessionStart inject was always one session stale. Same-class bug as the stale-OK skip (LLM output never reached the vault). Plus: `handle_session_start` sync fallback only fired when `pending and not cache_exists`; with stale cache present, X-close left the user one session behind on every cycle. Fixes: `_run_brief_regen` → `recall_briefing.brief_and_cache`; `_run_identity_refresh_if_due` → `mnemos.identity.refresh`; sync-fallback gate relaxed to `if pending:`; defensive `_strip_briefing_preamble` for LLM preamble leakage. 4 new tests, 1 prior test renamed-and-retargeted; **556 pass** (+3 net vs 553).
- Test suite **556 pass** (+27 vs 529 baseline; +11 vs 545: 6 supersede + 4 SessionEnd-persistence + 1 retargeted test for the X-close stale-cache case)
...
3. Verify `git status` clean + `origin/main == HEAD` (post-2026-04-29 SessionEnd-persistence fix) + pytest **556** pass
...
**Smoke verification on resume (worth doing before publish):** after the `/exit` that ended *this* writing session, three things should have happened live:
1. `supersede_stale_refine_if_needed` (from 2026-04-28's `7959ada`) should have superseded any stale OK row for this JSONL if its mtime exceeds the prior Session/.md by 60s.
2. `_run_brief_regen` (now via `brief_and_cache` in `2cf43f9`) should have written a fresh `<vault>/.mnemos-briefings/C--Projeler-mnemos-v1-1.md` with mtime ≥ /exit moment, body starting with `**Current State:**` (no preamble).
3. `_run_identity_refresh_if_due` should have either skipped (gate not met) or written a fresh `_identity/L0-identity.md` with advanced `last_refreshed`.

If all three are present, the v1.2.1 scope is end-to-end-validated and PyPI publish can proceed with confidence.
